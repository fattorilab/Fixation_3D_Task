...
public class Saver : MonoBehaviour
{
    ...
    {
        bool wants2saveData = EditorUtility.DisplayDialog("SAVE DATA", "Do you want to save frame data?", "Yes", "No");
        if (!wants2saveData)
        {
            // Ask user if she's sure not to save
            string The_End = ("Everything not saved will be lost");
    ...
    {
        bool wants2saveVideos = EditorUtility.DisplayDialog("SAVE VIDEO", "Do you want to save the video?", "Yes", "No");
        if (!wants2saveVideos)
        {
            // Ask user if she's sure not to save
            string The_End = ("Everything not saved will be lost");
    ...
    private void addDataPerFrame()
    {
        ...
        {
            PerFrameData[(PerFrameData.Count - 1)].Add((PupilDataStream.PupilTimeStamps*1000d).ToString("F5")); //timestamps are originally double and seconds
        ...
        PerFrameData[(PerFrameData.Count - 1)].Add("NaN"); // Timestamp
        PerFrameData[(PerFrameData.Count - 1)].Add("NaN"); // CenterRightPupilPx[0]
        PerFrameData[(PerFrameData.Count - 1)].Add("NaN"); // CenterRightPupilPx[1]
        PerFrameData[(PerFrameData.Count - 1)].Add("NaN"); // CenterLeftPupilPx[0]
        PerFrameData[(PerFrameData.Count - 1)].Add("NaN"); // CenterLeftPupilPx[1]
        PerFrameData[(PerFrameData.Count - 1)].Add("NaN"); // DiameterRight
        PerFrameData[(PerFrameData.Count - 1)].Add("NaN"); // DiameterLeft
        }
    ...
    private void saveAllData(string delimiter)
    {
        ...
        string eyes_vars = "pupil_timestamp; px_eye_right; py_eye_right; px_eye_left; py_eye_left; " +
                                "eye_diameter_right; eye_diameter_left";
        ...
        // Get parameters from public fields of the scripts of the experiment
